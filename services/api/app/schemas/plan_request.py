...
class PlanRequest(BaseModel):
    ...
    timezone: str = Field(min_length=1, description="IANA name, e.g. Europe/Istanbul. Never a fixed offset — that breaks twice a year.")
    days: int | None = Field(default=None, ge=1, le=16, description="Forecast horizon in days.")
    profile: ActivityProfile = Field(description="What a person wants from the weather before they will go outside. Extracted from natural language once, confirmed by the user, then read by the scoring engine on every request.")
