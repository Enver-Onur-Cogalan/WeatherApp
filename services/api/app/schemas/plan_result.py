...
class DaySummary(BaseModel):
    ...
    temp_min_c: float
    temp_max_c: float
    weather_code: int = Field(description="The day's headline condition: the most significant WMO code during daylight hours. WMO orders codes roughly by severity, so a thunderstorm outranks cloud and a 03:00 fog never headlines a sunny day.")
...
class PlanResult(BaseModel):
    ...
    days: list[DaySummary] = Field(description="One summary per day, so the screen can answer 'what is Thursday like' without the client aggregating hours itself.")
    now_index: int | None = Field(default=None, ge=0, description="Index into `hours` for the hour happening now, or null when the forecast does not cover it. Current conditions are hours[now_index] — the server resolves 'now' so the client never has to reason about the location's timezone.")
